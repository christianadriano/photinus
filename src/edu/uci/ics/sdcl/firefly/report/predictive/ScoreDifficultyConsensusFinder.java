...
import edu.uci.ics.sdcl.firefly.util.MicrotaskMapUtil;
	...
	public class ScoreDifficultyConsensusFinder {
		...
		private Outcome computeDataPoint(AnswerData answerData, Consensus predictor, HashMap<String, QuestionLinesMap> lineMapping) {
			...
			outcome.correct_NO_Answers = AnswerData.countCorrectAnswers(answerData.answerMap, answerData.bugCoveringMap, Answer.NO);
			outcome.correct_YES_Answers = AnswerData.countCorrectAnswers(answerData.answerMap, answerData.bugCoveringMap, Answer.YES);
			outcome.total_YESNO_Answers = answerData.getTotalAnswers() - AnswerData.count(answerData.answerMap, Answer.I_DONT_KNOW);
